...
from lib.utils import make_divisible, int2list
from .units.mobileUnit import MBInvertedConvLayer


class OFAMobileNetV3(nn.Module):
    def __init__(self, n_classes=1000, bn_param=(0.1, 1e-5), dropout_rate=0.1, width_mult=1.0,
        ...
        # width multiplier
        base_stage_width = [make_divisible(t * width_mult) for t in base_stage_width]
        ...
        first_conv = nn.Conv2d(3, base_stage_width[0], kernel_size=3, stride=2)
        first_block = MBInvertedConvLayer(base_stage_width[0], base_stage_width[0], 3, stride_stages[0],
        ...
        blocks = [first_block]
        _block_index = 1
        feature_dim = base_stage_width[0]
