...
class SAConv(nn.Module):
    ...
    def reset_parameters(self):
        ...
        init.kaiming_normal_(self.project_conv.weight, mode='fan_out', nonlinearity='relu')
...
class SABottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, kernel_size=7, groups=1, base_width=64, heads=8):
        ...
        self.conv2 = nn.Sequential(
            SAConv(width, width, kernel_size=self.kernel_size, padding=padding, heads=heads),
            nn.BatchNorm2d(width),
            nn.ReLU(),
        )
