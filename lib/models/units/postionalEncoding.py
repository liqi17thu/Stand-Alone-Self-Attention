...
class PositionalEncoding(nn.Module):
    def __init__(self, out_channels, kernel_size, heads, bias=False, encoding='learnable', r_dim=256):
        ...
        if type == "learnable":
            self.encoding = LearnablePostionalEncoding(out_channels, kernel_size)
        elif type == "sine":
            self.encoding = SinePositionalEncoding(out_channels // heads)
        elif type == "xl":
            self.encoding = XLPositionalEncoding(out_channels, heads, r_dim, bias)
        else:
...
class LearnablePostionalEncoding(nn.Module):
    def __int__(self, out_channels, kernel_size):
